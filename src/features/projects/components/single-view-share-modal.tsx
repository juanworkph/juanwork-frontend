...
export const SingleViewShareModal = ({
  ...
  // Reset copied state when modal opens and set focus
  useEffect(() => {
    if (isOpen) {
      setCopied(false);
      
      // Focus copy button when modal opens
      setTimeout(() => {
  ...
  const handleCopyLink = async () => {
    try {
      ...
      toast.success("Link copied to clipboard", {
        description: "You can now share this project link with others",
      });
    } catch (error) {
  ...
  const handleShareWhatsApp = () => {
    const text = `Check out this project: ${projectName} - ${projectUrl}`;
    const url = `https://wa.me/?text=${encodeURIComponent(text)}`;
    window.open(url, "_blank", "width=600,height=400");
  };

  const handleShareEmail = () => {
    const subject = `Check out this project: ${projectName}`;
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                aria-label="Project URL for sharing"
                aria-describedby="copy-link-description"
              />
              <span id="copy-link-description" className="sr-only">
                Copy this link to share the project with others
              </span>
              <button
                ref={copyButtonRef}
