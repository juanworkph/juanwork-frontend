...
interface SingleViewBiddingFormProps {
  project: ProjectDetails;
  existingBid?: Bid;
  onSubmit: (bid: BidFormData) => Promise<void>;
}

export const SingleViewBiddingForm = ({
  project,
  existingBid,
  onSubmit,
}: SingleViewBiddingFormProps) => {
  // Form state
  ...
  const handleFieldChange = useCallback(
    (field: string, value: string) => {
      ...
      if (touched[field]) {
        // Use setTimeout to debounce validation slightly
        setTimeout(() => {
          validateForm();
        }, 0);
      }
    ...
    <Card>
      <CardHeader className="p-4 sm:p-6">
        <CardTitle className="text-base sm:text-lg">
          {existingBid ? "Your Bid" : "Submit a Proposal"}
        </CardTitle>
        <CardDescription className="text-xs sm:text-sm">
          {existingBid
            ? "View or edit your submitted proposal"
